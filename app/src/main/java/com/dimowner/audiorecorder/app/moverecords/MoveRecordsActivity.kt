    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.playProgress.setOnSeekBarChangeListener(object : OnSeekBarChangeListener {
            override fun onProgressChanged(seekBar: SeekBar, progress: Int, fromUser: Boolean) {
                if (fromUser) {
                    val value =
                        AndroidUtils.dpToPx(progress * binding.waveformView.getWaveformLength() / 1000)
                            .toInt()
...
//				//TODO: Find a better way to convert px to mills here
    ...
    private fun showInfoDialog() {
        if (intent.hasExtra(PREF_KEY_SHOW_INFO) && intent.getBooleanExtra(
                PREF_KEY_SHOW_INFO,
                false
            )
        ) {
    ...
    private fun onPlayPanelUpdate(state: MoveRecordsPlayPanelState) {
        ...
        binding.waveformView.setWaveform(
            state.activeRecordData,
            state.playRecordDuration,
            state.playProgressMills
        )
    }
                ...
                .setListener(object : AnimatorListenerAdapter() {
                    override fun onAnimationEnd(animation: Animator) {
                        ...
                        binding.recyclerView.smoothScrollBy(
                            0,
                            (binding.touchLayout.height * k).toInt()
                        )
